...
### Wi-Fi Unavailable (`WIFI_UNAVAILABLE` - Figma node `2079:2300`) - M4-19
...
*AFTER matches the Figma design at canvas position x=-1561 (selection URL `node-id=10-29`). The design at that position is not wrapped in a single frame in the Figma file - it is the union of five sibling top-level groups (`2073:1862` banner, `2073:1690` countdown + Continue + info icon area, `2073:1773` Back button, `2073:1717` Retry WiFi button, `2073:1661` Setting button) sitting on top of the `10:29` background rectangle. The TARGET panel above is a composite of those five rendered pieces, positioned by their `absoluteBoundingBox` figma coordinates so it shows the screen design without the info-modal overlay. The info-modal (Figma node `2079:2300`) that appears when the user taps the orange `!` button is a follow-up; its render is preserved in this report folder as `figma-modal-pending-2079-2300.png`.*
